package com.meugenom.leetcode.UniqueEmailAddresses;
 ...
 * email address,
 * mail sent there will be forwarded to the same address without dots in the
 * local name.
 ...
 * will be ignored. This allows certain emails
...
public class UniqueEmailAddresses {
	...
	public static int numUniqueEmails(String[] emails) {
		...
		for (int i = 0; i < emails.length; i++) {
			String name = emails[i].split("@")[0];
			String domen = emails[i].split("@")[1];
			name = name.split("\\+")[0];
			name = name.replace(".", "");
			unique.add(name + "@" + domen);
		}
