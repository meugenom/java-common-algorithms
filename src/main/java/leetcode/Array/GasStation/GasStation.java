...
/**
 * Issue 134 "Gas Station" medium
 * 
 * There are n gas stations along a circular route, where the amount of gas at the ith 
 * station is gas[i]. You have a car with an unlimited gas tank and it costs cost[i] of gas 
...
public class GasStation {
	...
	public int canCompleteCircuit(int[] gas, int[] cost) {
		...
		// If totalCost is greater than totalGas,
		// it is not possible to complete the circuit
		...
		// Iterate through all the gas stations
		for (int i = 0; i < gas.length; i++) {
			// Add the difference between gas
			// and cost at each station to remainsGas
			remainsGas = remainsGas + (gas[i] - cost[i]);
			// If remainsGas becomes negative,
			// set start to the next station and reset remainsGas to 0
			if (remainsGas < 0) {
				start = i + 1;
				remainsGas = 0;
			}
